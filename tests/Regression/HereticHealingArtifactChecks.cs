...
using ManagedDoom;
static class HereticArtifactChecks
{
    ...
    public static void Verify(string root)
    {
        ...
        foreach (var type in new[] { HereticActorType.MT_MISC3, HereticActorType.MT_ARTISUPERHEAL })
        {
            ...
            var artifact = type == HereticActorType.MT_MISC3 ? HereticArtifact.QuartzFlask : HereticArtifact.MysticUrn;
            for (var i = 0; i < 16; i++) Check(s.GiveArtifact(artifact), "Inventory filled too early.");
            Check(!s.GiveArtifact(artifact), "Healing artifact exceeded sixteen.");
            Touch(s, pickup.Body); Check(s.Actors.Contains(pickup), "Full inventory consumed map artifact.");
            Check(!s.UseArtifact(artifact), "Artifact consumed at full health.");
            s.DamageEnvironment(30);
            Check(s.UseArtifact(artifact) && s.State.Health == (artifact == HereticArtifact.QuartzFlask ? 95 : 100)
            ...
            Check((artifact == HereticArtifact.QuartzFlask ? s.State.QuartzFlasks : s.State.MysticUrns) == 16, "Artifact stored count differs.");
        ...
        stepper.Advance(0.001, new HereticCommand { UseArtifact = HereticArtifact.QuartzFlask }, commands.Add);
        ...
        var heal = new HereticWorldSession(content); heal.GiveArtifact(HereticArtifact.QuartzFlask); heal.GiveArtifact(HereticArtifact.QuartzFlask);
        ...
        heal.GiveArtifact(HereticArtifact.MysticUrn);
        heal.Tick(new HereticCommand { UseArtifact = HereticArtifact.MysticUrn });
        Check(heal.State.Health == 100 && heal.State.MysticUrns == 0, "Urn did not heal/cap/consume.");
        heal.DamageEnvironment(100);
        Check(!heal.UseArtifact(HereticArtifact.QuartzFlask) && heal.State.QuartzFlasks == 1 && !heal.GiveArtifact(HereticArtifact.MysticUrn), "Healing inventory resurrected or granted to dead player.");
