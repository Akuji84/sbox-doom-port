...
namespace MeltySynth
{
    ...
    public sealed class SoundFont
    {
        ...
        public SoundFont(string path)
        {
            ...
            throw new Exception("Path-based SoundFont loading is not supported in this s&box build.");
        }
        ...
        private void Load(Stream stream)
        {
            using (var reader = new BinaryReader(stream, Encoding.ASCII, true))
            {
                ...
                if (chunkId != "RIFF")
                {
                    throw new Exception("The RIFF chunk was not found.");
                }
                ...
                if (formType != "sfbk")
                {
                    throw new Exception($"The type of the RIFF chunk must be 'sfbk', but was '{formType}'.");
                }
        ...
        private void CheckSamples()
        {
            ...
            foreach (var sample in sampleHeaders)
            {
                ...
                {
                    throw new Exception($"The start position of the sample '{sample.Name}' is out of range.");
                }
                if (!(0 <= sample.StartLoop && sample.StartLoop < sampleCount))
                {
                    throw new Exception($"The loop start position of the sample '{sample.Name}' is out of range.");
                }
                if (!(0 < sample.End && sample.End <= sampleCount))
                {
                    throw new Exception($"The end position of the sample '{sample.Name}' is out of range.");
                }
                if (!(0 <= sample.EndLoop && sample.EndLoop <= sampleCount))
                {
                    throw new Exception($"The loop end position of the sample '{sample.Name}' is out of range.");
                }
            }
        }
        ...
        private void CheckRegions()
        {
            ...
            foreach (var instrument in instruments)
            {
                foreach (var region in instrument.RegionArray)
                {
                    ...
                    {
                        throw new Exception($"The start position of the sample '{region.Sample.Name}' in the instrument '{instrument.Name}' is out of range.");
                    }
                    if (!(0 <= region.SampleStartLoop && region.SampleStartLoop < sampleCount))
                    {
                        throw new Exception($"The loop start position of the sample '{region.Sample.Name}' in the instrument '{instrument.Name}' is out of range.");
                    }
                    if (!(0 < region.SampleEnd && region.SampleEnd <= sampleCount))
                    {
                        throw new Exception($"The end position of the sample '{region.Sample.Name}' in the instrument '{instrument.Name}' is out of range.");
                    }
                    if (!(0 <= region.SampleEndLoop && region.SampleEndLoop <= sampleCount))
                    {
                        throw new Exception($"The loop end position of the sample '{region.Sample.Name}' in the instrument '{instrument.Name}' is out of range.");
                    }
                    ...
                    switch (region.SampleModes)
                    {
                        ...
                        case LoopMode.LoopUntilNoteOff:
                            break;
                        default:
                            throw new Exception($"The sample '{region.Sample.Name}' in the instrument '{instrument.Name}' has an invalid loop mode.");
                    }
                }
            }
        }
