...
namespace MeltySynth
{
    ...
    public sealed class MidiFile
    {
        ...
        public MidiFile(string path)
        {
            ...
            throw new Exception("Path-based MIDI loading is not supported in this s&box build.");
        }
        ...
        private void Load(Stream stream, int loopPoint, MidiFileLoopType loopType)
        {
            using (var reader = new BinaryReader(stream, Encoding.ASCII, true))
            {
                ...
                if (chunkType != "MThd")
                {
                    throw new Exception($"The chunk type must be 'MThd', but was '{chunkType}'.");
                }

                var size = reader.ReadInt32BigEndian();
                if (size != 6)
                {
                    throw new Exception($"The MThd chunk has invalid data.");
                }
        ...
        private static (List<Message>, List<int>) ReadTrack(BinaryReader reader, MidiFileLoopType loopType)
        {
            ...
            if (chunkType != "MTrk")
            {
                throw new Exception($"The chunk type must be 'MTrk', but was '{chunkType}'.");
            }
        ...
        private static int ReadTempo(BinaryReader reader)
        {
            ...
            if (size != 3)
            {
                throw new Exception("Failed to read the tempo value.");
            }
