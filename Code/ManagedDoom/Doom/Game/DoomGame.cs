	...
	public sealed class DoomGame
	{
		...
		private void DoLoadGame()
		{
			...
			catch
			{
				if (world != null)
				{
					world.ConsolePlayer.SendMessage("Load game failed.");
				}
			}
		}
		...
		{
			gameAction = GameAction.Nothing;
			if (world == null || gameState != GameState.Level)
			{
				return;
			}
			...
			{
				world.ConsolePlayer.SendMessage("Save game failed.");
			}
		}
		...
		internal void DoLoadLevel_Internal()
		{
			DoLoadLevel();
		}

		internal void RestoreAfterLoad(int restoredGameTic)
		{
			gameAction = GameAction.Nothing;
			gameState = GameState.Level;
			intermission = null;
