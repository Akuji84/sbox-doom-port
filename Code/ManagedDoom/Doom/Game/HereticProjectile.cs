...
namespace ManagedDoom
{
    public sealed partial class HereticProjectile : IHereticActorActions
    {
        ...
        public void Execute(HereticAction action, HereticActorState actor)
        {
            ...
            if (SupportsSorcerer(action)) { ExecuteSorcerer(action); return; }
        ...
        internal bool Contact(Mobj target)
        {
            ...
            if (Type == HereticActorType.MT_RIPPER)
            {
                ...
                var ripDamage = ((session.World.Random.Next() & 3) + 2) * def.Damage;
                if (session.SameMonsterType(target, HereticActorType.MT_HEAD)) ripDamage = session.World.Random.Next() & 1;
                session.DamageTestEnemy(target, ripDamage, inflictor: Body);
                return true;
            }
