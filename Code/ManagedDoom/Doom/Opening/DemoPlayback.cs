...
namespace ManagedDoom
{
    public sealed class DemoPlayback
    {
        ...
        {
            var normalizedDemoName = demoName.ToUpperInvariant();
            if (content.Wad.GetLumpNumber(normalizedDemoName) != -1)
            {
                demo = new Demo(content.Wad.ReadLump(normalizedDemoName));
            }
            else
            {
                var lumpName = normalizedDemoName + ".LMP";
