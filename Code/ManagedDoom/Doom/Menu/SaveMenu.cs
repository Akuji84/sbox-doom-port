...
namespace ManagedDoom
{
    public sealed class SaveMenu : MenuDef
    {
        ...
        {
            var description = new string(items[slotNumber].Text.ToArray());
            if (Menu.Doom.SaveGame(slotNumber, description))
            {
                Menu.SaveSlots[slotNumber] = description;
                Menu.Close();
                lastSaveSlot = slotNumber;
            }
            else
            {
                Menu.NotifySaveUnavailable();
            }
            Menu.StartSound(Sfx.PISTOL);
        }
