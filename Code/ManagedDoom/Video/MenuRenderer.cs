...
namespace ManagedDoom.Video
{
    public sealed class MenuRenderer
    {
        ...
        public void Render(DoomMenu menu)
        {
            ...
            var bugReport = menu.Current as BugReportMenu;
            if (bugReport != null)
            {
        ...
        private void DrawBugReportMenu(BugReportMenu bugReport)
        {
            var title = bugReport.Title[0];
            var titleScale = screen.Width / 320;
            var titleChars = title.ToCharArray();
            var titleX = (screen.Width - screen.MeasureText(titleChars, titleScale)) / 2;
            screen.DrawText(titleChars, titleX, titleScale * 16, titleScale);

            for (var i = 0; i < bugReport.Labels.Count; i++)
            {
                DrawMenuText(bugReport.Labels[i].ToCharArray(), bugReport.LabelX[i], bugReport.LabelY[i]);
            }

            DrawTextBoxMenuItem(bugReport.Contact, bugReport.Menu.Tics);
            DrawTextBoxMenuItem(bugReport.Map, bugReport.Menu.Tics);
            DrawTextBoxMenuItem(bugReport.Details, bugReport.Menu.Tics);
        ...
        private void DrawMenuItem(DoomMenu menu, MenuItem item)
        {
            ...
            var text = item as TextMenuItem;
            if (text != null)
            {
                DrawTextMenuItem(text);
            }
        ...
        {
            var menuScale = screen.Width / 320;
            screen.DrawText(
                item.Text.ToCharArray(),
                menuScale * item.ItemX,
                menuScale * (item.ItemY + item.YOffset),
                menuScale * item.TextScale);
        }
        ...
        private void DrawText(IReadOnlyList<string> text)
        {
            ...
            for (var i = 0; i < text.Count; i++)
            {
                ...
                var x = (screen.Width - screen.MeasureText(line, scale)) / 2;
                var y = (screen.Height - height) / 2 + 7 * scale * (i + 1);
                screen.DrawText(line, x, y, scale);
            }
        }
